...
- Desktop: a full-viewport hero (620px minimum), a centered 1440px copy container, and an independently positioned right-half carousel. Its center sits at 92% of that half; radius follows the smaller of 32% of its height (220px floor), 55% of its width, and 410px. Card width is 1.05 times the radius with a 250px floor. The shared site shell elsewhere is unchanged.
- Ten evenly spaced slots follow a complete circle. The seven existing decorative previews repeat across those slots. Cards use 16:9 artwork, 12px corners, subtle X/Y perspective, angle-based stacking, and broad shadows. Only the right edge fades; no top/bottom mask. The SVG previews are recomposed for 16:9 so their labels and artwork are not cropped.
- Idle motion is 0.1 radians/second. Native scroll in either direction adds 0.004 radians/second per pixel to momentum, capped at 5 radians/second and decayed exponentially at 1.6/s. This matches the reference's angular response rather than the previous 56-second partial arc. No wheel interception or scroll hijacking.
- Entrance begins at 800ms with a 3 radians/second launch impulse. Cards fan from a shared right-hand origin with 80ms stagger, 850ms cubic ease-out, and a radius that expands from 45% to 100% over 1.9s. Copy retains its 24px staggered rise, now with the reference's one-second duration. First-load assembly settles around 2.7s; content never waits on a blocking loader.
- The renderer uses one requestAnimationFrame owner for card transform/opacity; CSS owns responsive sizing and hover. Hover and keyboard focus pause the carousel. Hidden tabs and off-screen stages suspend the frame loop. Reduced motion and JavaScript-off retain a server-rendered still. Tablet/mobile retain their responsive in-flow media slot, with smaller mobile geometry.
...
The hero typography now follows the owner’s reference: a restrained two-line name with a near-black first name and gray surname, supporting description, and only the selected-work/contact CTA pair. The separate value headline and hero social links are omitted; social destinations remain available in contact and the footer.
